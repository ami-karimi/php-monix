      ...
      if (($this->pages) > 1) {
        ...
        if ($this->page != 1) {
          if ($this->_showFirstAndLast) {
            $plinks[] = ' <a href="?route=manage_plugin&page=1'.$queryUrl.'">&laquo;&laquo; ابتدا </a> ';
          }
          $plinks[] = ' <a href="?route=manage_plugin&page='.($this->page - 1).$queryUrl.'">&laquo; قبلی</a> ';
        }
        ...
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            ...
            $links[] = ' <a href="?route=manage_plugin&page='.$j.$queryUrl.'">'.$j.'</a> '; // add the link to the array
          }
        }
        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a href="?route=manage_plugin&page='.($this->page + 1).$queryUrl.'"> بعدی &raquo; </a> ';
          if ($this->_showFirstAndLast) {
            $slinks[] = ' <a href="?route=manage_plugin&page='.($this->pages).$queryUrl.'"> آخرین &raquo;&raquo; </a> ';
          }
        }
