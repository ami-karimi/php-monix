...
if (!class_exists('Hooks')){
  ...
  */
  class Hooks
  {
    ...
    function Get_PluginShortName($dirname){
		$db = new Db();
		$data = $db->row("SELECT pr_key FROM  td_personaliz WHERE pr_setting = '{$dirname}' and pr_group = 'plugin' ");
	    return $data['pr_key'];
	}
    // Function Plugin
	function Monix_admin_PluginUrl($dir){
	  $dir_name = str_replace('/tk-panel/','',basename($dir));
	  $ShortName = Get_PluginShortName($dir_name);
	  return Mo_url(AdminTarget().'/Admin_plugin').'?Admin&short_name='.$ShortName.'&plugin='.$dir_name;
	}
	
	function Monix_Root_PluginDir($dir){
	  $dir_name = str_replace('/tk-panel/','',basename($dir));
	  return PLUGIN_DIR.$dir_name."/";
	}
	
	function Monix_set_preference($key,$value,$grup){
     $db = new Db();
	 $data = $db->row("SELECT pr_key FROM  td_personaliz WHERE pr_key = '{$key}' and pr_group = '{$grup}' ");	
      if($data['pr_key'] !== $key){
	  $db->query("INSERT INTO td_personaliz(pr_key,pr_value,pr_group) VALUES(:pr_key,:pr_value,:pr_group)",
	   array(
	   "pr_key"=> $key,
	   "pr_value"=> $value,
	   "pr_group"=> $grup
       ...
       $db->query("UPDATE  td_personaliz SET pr_key = :pr_key,pr_value = :pr_value ,pr_group = :pr_group WHERE pr_key = '{$key}' and pr_group = '{$grup}' ",
	   array(
	   "pr_key"=> $key,
	   "pr_value"=> $value,
	   "pr_group"=> $grup
	...
	function Monix_SrcPluginUrl($dir_s){
	  $dir = realpath(dirname($dir_s));
	  return "http://".$_SERVER['HTTP_HOST']."/Connect/Plugin/".basename($dir)."/".basename($dir_s);
	}
	function Monix_BaseSrcPluginUrl($dir_s){
	  $dir = realpath(dirname($dir_s));
	  return "http://".$_SERVER['HTTP_HOST']."/Connect/".basename($dir)."/".basename($dir_s);
	}
	function Monix_get_preference($key,$grup){
     $db = new Db();
	 $data = $db->row("SELECT pr_value FROM  td_personaliz WHERE pr_key = '{$key}' and pr_group = '{$grup}' ");	
	 return $data['pr_value'];
	}
	function Monix_delete_preference($key,$group){
	 $db = new Db();
	 $db->query("DELETE FROM td_personaliz WHERE pr_key = :id and pr_group = :pr_group ", array("pr_group"=>$group,"id"=> $key));	
	}
	function Monix_delete_preferenceGroup($group){
	 $db = new Db();
	 $db->query("DELETE FROM td_personaliz WHERE pr_group = :pr_group ", array("pr_group"=>$group));	
	}

	function AdminName(){
		 if(isset($_SESSION['admin_id'])){
			  $db = new Db();
			  $data = $db->row("SELECT am_fname FROM  td_asmin WHERE id_am = '".$_SESSION['admin_id']."' ");	
	          return $data['am_fname'];
		 }else{
			 return 0;
		 }
	}
	function AdminNameById($id){
			  $db = new Db();
			  $data = $db->row("SELECT am_fname,id_am FROM  td_asmin WHERE id_am = '{$id}' ");	
			  if($data['id_am'] == $id){
	          return $data['am_fname'];
			  }else{
				  return 'نامشخص';
    ...
    if(!isset($_GET['install']) or !isset($_GET['Uninstall'])){
    // include Plugin
    $data  = $db->query("SELECT pr_value,pr_setting FROM td_personaliz WHERE pr_group = 'plugin' and pr_value = '1' ");	
	 if(count($data) > 0){
 	  foreach($data as $row){
		  require_once(PLUGIN_DIR.$row['pr_setting'].'/index.php');
	 }
	}
	}
    function Check_InstallPlugin($short_Name){
		...
		$data = $db->row("SELECT pr_key FROM td_personaliz WHERE pr_key = '{$short_Name}'  and pr_group = 'plugin' "); 
         if($data['pr_key'] == $short_Name){
			 $data  = $db->row("SELECT pr_value FROM td_personaliz WHERE pr_key = :id", array("id"=> $short_Name));
			 if($data['pr_value'] == '1'){
				return 1;
  ...
  if(isset($_GET['AjaxLoadPlugin'])){
	  $pluginDir = htmlentities($_GET['AjaxLoadPlugin']);
	  $short_name = Get_PluginShortName($pluginDir);
	  $check_Install_Plugin = Check_InstallPlugin($short_name);
	  if(is_dir(PLUGIN_DIR.$pluginDir) and preg_match("/^[a-zA-Z_]*$/", $pluginDir) == 1 and $check_Install_Plugin !== 0){
		require_once(PLUGIN_DIR.$pluginDir."/index.php");  
	  }
  }

	   function Monix_ReqPlugin($path,$funcName,$filename){ 
		   if(isset($_GET['install']) and $_GET['plugin'] == $filename){
		     include_once "{$path}/index.php";
			 call_user_func($funcName);  
		   }
	   }
       
       function  Monix_UnistallPlugin($path,$funcName,$filename){
		   if(isset($_GET['Uninstall']) and $_GET['plugin'] == $filename){
		     include_once "{$path}/index.php";
			 call_user_func($funcName);   
		   }
       }
...
function MOHeader(){
 ...
 function Monixset_lang($dir,$def){ 
	define('LANG_FOLDER',THEME_DEFAULT."/$dir");
	define('LANG_DEFINE',$def);
 }

 function Monix_url_route($name,$parm){
   global $router; 
   return $router->route($name,$parm);
 }
 function Monix_plugin_folder($file){
    $dir = preg_replace('|/+|','/', str_replace('\\','/',dirname($file)));
    $dir = preg_replace('#^.*'.CONNECT_DIR.'/Plugin\/#','',$dir) . "/";
    return $dir;
 }