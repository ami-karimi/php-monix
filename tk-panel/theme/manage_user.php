    <section id="main-content">
     ...
     <section class="wrapper">
		...
		<div class="panel-body">
									 ...
									 <form method="POST" class="form-inline" action="<?=Mo_url(AdminTarget().'/manage_user');?>" role="form">
								   <?php if(isset($text)){?> <div class="tags"><div style="margin-top: 1px;" onclick="window.location='<?=Mo_url(AdminTarget().'/Manage_news');?>' " class="tag_remove"><i class="icon-remove"></i></div><label style="font-size: 13px;"><?php if($text){echo $text;}?></label></div> <?php }?>
                                    <div class="form-group">
                                        <input class="form-control" name="text" value="<?php if(isset($text)){echo $text;}?>" placeholder="نام،نام کاربری جهت جستجو" type="text">
                                    </div> 
                                ...
                                <tbody>  
								 <?php foreach($data as $row){?>
								   <tr> 
                                        ...
                                        <td> 
										<a onclick="confirm_box('ایا میخواهید این کاربر را  حذف نمایید؟ در صورت حذف کاربر  تمام فایل ها و درخواست های این کاربر  حذف خواهد شد','<?=Mo_url(AdminTarget().'manage_user');?>?delete_id=<?=$row['id_user'];?>');" class="btn btn-danger  btn-xs"><i class="icon-trash"></i></a>
										<a class="btn btn-info  btn-xs" href="<?=Mo_url(AdminTarget().'/show_user');?>?user_id=<?=$row['id_user'];?>"><i class="icon-pencil"></i></a>
