     ...
     <tr> 
	  ...
	  <tbody> 
		 ...
		 <tr>
		    <td class="fa-number"><a class="btn btn-xs btn-primary" href="<?=MO_url(AdminTarget()."/manage_tiket/show_tiket/".$tiket['tk_code']);?>"><?=$tiket['tk_code']."#-".$tiket['tk_title'];?></a></td>
          ...
          <section class="panel">
		    ...
		    <div class="panel-body">
               ...
               <div class="form-group">
                ...
                <a onclick="confirm_box('ایا از حذف این درخواست اطمینان دارید؟ با حذف  تیکت تمامی  فایل ها و پیام های مربوط به این درخواست  حذف خواهد شد ','<?=MO_url(AdminTarget()."/manage_tiket");?>?delete_id=<?=$row['tk_code'];?>');"  href="#" class="pull-left" style="color:red;"> حذف درخواست </a>
	        </div>
	  ...
	  <tbody> 
       <?php foreach($tiket_all_user as $row){ ?>
		 <tr>
           ...
           <td><a class="btn btn-info  btn-xs" href="<?=MO_url(AdminTarget()."/manage_tiket/show_tiket/".$row['tk_code']);?>"><i class="icon-eye-open"></i></a></td>
         </tr>		 
        <?php }?>  
